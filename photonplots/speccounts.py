...
# Let's leave in the zeroes so they have the same length
...
#        filter_area = filter_area[nonzero]
    ...
    print(len(filter_A[0]),len(filter_A[1]),len(filter_A[2]),len(filter_A[3]),len(filter_A[4]),len(filter_A[5]))
    filter_array = np.array([filter_A[0],filter_A[1],filter_A[2],filter_A[3],filter_A[4],filter_A[5]])
    print(len(filter_WL[0]),len(filter_WL[1]),len(filter_WL[2]),len(filter_WL[3]),len(filter_WL[4]),len(filter_WL[5]))

    filter_wave = np.array([filter_WL[0],filter_WL[1],filter_WL[2],filter_WL[3],filter_WL[4],filter_WL[5]])
    
    wavez=np.array(wavez)
    fluxz=np.array(fluxz)
    bad =np.where(np.isnan(fluxz))[0]
    good=np.where(~np.isnan(fluxz))[0]
    #print(good)
    #fig, ax = plt.subplots()
    #ax.plot(wavez[good], fluxz[good])

    fluxz[bad]=np.interp(wavez[bad],wavez[good], fluxz[good])  
    ...
    for x in range(len(filtercurves)):    
        ...
        #fig, ax = plt.subplots()
        #ax.plot(wavez, sp_ea*fluxz*wavez/hc)
    ...
    print("returns counts_array--here are the magnitudes")
    print(mag_array)    
    return counts_array;
